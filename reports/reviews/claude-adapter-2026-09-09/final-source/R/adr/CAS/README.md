...
**Latest product service update, 2026-09-07:** [CAS-007](ADRL-CAS-007.md) record the applied session,
...
**Earlier foundation update, 2026-09-07:** [CAS-007](ADRL-CAS-007.md) records Messages response
rendering moved into the protocol package and an import-order fix verified by fresh-process
tests. Error and retry policy remains unchanged. This extraction does not prove another protocol
or live gateway behavior. See the [synchronization record](../../reports/adrl-register-sync-2026-09-07.md).
The review table below retains its historical scope.
...
| ADRL-CAS-006 | Record the served model, not only the served rung | AMEND | D2 Tested → D2 Tested | Caches and signatures follow the model, not the rung; the record needs provenance (gateway may echo the alias) and an explicit state-loss marker.  <!-- taxonomy-sync:claude-adapter:ADRL-CAS-006 --> [2026-09-09 offline candidate](../../reports/reviews/claude-adapter-2026-09-09/report.md); no grade change |
| ADRL-CAS-007 | Terminal failure surfaced; ADRL owns zero retries | AMEND | D2 Tested → D2 Tested | "Another automatic retry" needed a layer: ADRL zero, gateway bounded and constrained to rung/pin/streaming rules; "surfaced" must be a protocol error, never a synthetic assistant message.  <!-- taxonomy-sync:claude-adapter:ADRL-CAS-007 --> [2026-09-09 offline candidate](../../reports/reviews/claude-adapter-2026-09-09/report.md); no grade change |
...
| ADRL-CAS-009 | Action-effect provenance and authorization boundary (Proposed) | PROPOSED (new) | D0 | 2026-09-09 [Overview restoration only](../../reports/reviews/claude-adapter-2026-09-09/overview-restoration.json); existing INDEX scope, no implementation or promotion. |
...
ADRL-CAS-003, ADRL-CAS-007 gain scoped producer/consumer integration evidence. See the [repair report](../../reports/reviews/outcome-contract-2026-09-09/report.md). Fable critique and recheck complete for RV-01, no maturity promotion; historical and remaining learning gaps are explicit.
