...
Rejected bound requests receive native Messages error objects, while product operations use typed local API errors. No model retries were added. A disconnected event client can retry the same envelope safely; this does not authorize replay of model calls or tool actions.
...
Native Messages error and synthetic-response rendering now lives in
`adrl-core/src/adrl/wire/profiles/messages_responses.py`; `proxy/errors.py` retains compatible
exports. The move preserves the existing terminal-error and retry policy. A circular import
found during standalone capability discovery was corrected, and three fresh-process import
tests were added. The final suite passed 463 tests; this does not establish live gateway retry
budgets or another protocol's error compatibility.

The separate product namespace returns an explicit 501 for its unimplemented operations and
never forwards those payloads to a model gateway. This product API response is distinct from
native Messages errors. See the [implementation record](../../reports/adrl-product-foundation-implementation-2026-09-07.md)
for code and validation scope. The historical follow-ups below remain separate acceptance work.
...
|---|---|---|
| 2026-09-09 | <!-- taxonomy-sync:claude-adapter:ADRL-CAS-007 --> [Offline candidate](../../reports/reviews/claude-adapter-2026-09-09/report.md); Finite experiment transport performs no retry, redirect or health forwarding. Attempt reservation precedes dispatch, failed attempts consume allowance, and response lifetime retains the single-active slot. Terminal failures use existing error rendering; response bytes are unchanged. Process-local counters do not establish restart-safe run custody. | Prior decision wording preserved; no grade change |
...
| 2026-09-07 | Recorded protocol-owned Messages response rendering, namespace isolation and the tested import fix | Decision text and retry policy unchanged; no extraction evidence previously recorded |
...
<!-- taxonomy-sync:claude-adapter:ADRL-CAS-007 --> Finite experiment transport performs no retry, redirect or health forwarding. Attempt reservation precedes dispatch, failed attempts consume allowance, and response lifetime retains the single-active slot. Terminal failures use existing error rendering; response bytes are unchanged. Process-local counters do not establish restart-safe run custody. [Evidence and limits](../../reports/reviews/claude-adapter-2026-09-09/report.md). Formal grades and status unchanged.
