...
The proposed initial lab-derived K0 knowledge snapshot adds an explicit compatibility matrix to the artifact design: harness/config, protocol/adapter, model/deployment identity, task/context scope, tools/environment and feature/verifier/policy versions. A candidate manifest records unavailable immutable provider revisions and evidence limits honestly. Version pinning does not itself grant learning admission or release authority. See the [lab proposal](../../design/adrl-experiment-lab-plan-2026-09-08.md). This is a pending planning application; decision text, status and maturity remain unchanged.

## W7.0a scoped application and evidence, 2026-09-08

Runtime feature semantics advance from features-v1 to features-v2. The existing offline learning contract intentionally stays v1; exact-version dataset validation therefore rejects new v2 rows until a compatible contract is separately qualified. Exploration startup now rejects a contract/live-feature mismatch. No historical artifact is relabeled, no compatibility is assumed and no artifact/data admission or training occurs.
...
Verifier proposals now carry a schema version, proposal and proposer versions, optional predecessor reference, hypothesis, affected ADR IDs, two verifier plans, the exact canonical suite digest, required gain, repeats and budget. Encrypted start records preserve the full proposal and suite; trials preserve execution provenance; final reports identify evaluator and snapshot-executor source. A changed evaluator file at completion blocks a recommendation and the report retains the starting identity.

Hashes pin the reviewed inputs and local implementation files, not the truth of expected answers, an immutable dependency graph or a remote attestation. ADR references and predecessor references are metadata, not validated approval authority. This scoped proposal contract does not complete the broader learned-artifact manifest, compatibility validation or graduation loader.
