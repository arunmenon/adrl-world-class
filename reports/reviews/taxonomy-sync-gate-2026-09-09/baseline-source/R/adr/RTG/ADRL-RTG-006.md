...
With no classifier configured in the offline driver, uncertain cases used the permitted fallback. The 24 ordinary probes chose local 9 times, frontier 15 times and cheap_cloud zero times; cheap_cloud appeared under restrictions and in cascade. This diagnoses the configured fallback path, not organic traffic, an advisor defect or evidence that a middle-tier model would succeed.
...
The advisor still receives coarse decision-time features only within its existing middle-band scope. features-v2 changes the upstream verb classification and therefore which requests can enter that band. No classifier prompt, endpoint, authority or budget changed, and no classifier was run in this slice. Actual advisor behavior/quality under the new input population remains unqualified.
