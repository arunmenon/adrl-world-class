...
The composition root now refuses configured exploration when its learning contract feature version differs from current live feature semantics. With runtime features-v2 and the retained v1 contract, exploration cannot be enabled. Ordinary routing without an exploration artifact remains available. Existing epsilon, permitted-set, privacy and graduation rules are unchanged; compatible contract/artifact qualification remains a follow-up.
