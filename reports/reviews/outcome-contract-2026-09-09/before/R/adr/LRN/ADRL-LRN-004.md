...
The context-routing and MEM graph proposal makes decision-time snapshots explicit. A pending disposition must distinguish verified observations of earlier steps from the current action's outcome and future information; the current broad deny-list is not silently relaxed. Historical features must reflect what ADRL knew then, with frozen projection/source versions and privacy/erasure handling. New graph-derived learned features remain unimplemented and unadmitted. See the [proposal](../../design/adrl-context-graph-memory-proposal-2026-09-08.md) and [product roadmap](../../reports/adrl-product-roadmap-2026-09-08.md). Current decision wording, status and maturity are unchanged.

## W7.0a scoped application and evidence, 2026-09-08

features-v2 aggregates the strongest recognized lexical signal across the latest user request, while recognizing bounded mechanical phrases and retaining existing context/trajectory extraction. The exact new snapshot/version is written on decisions. Historical v1 snapshots remain intact. Quotes and negation are unresolved and may over-route. No new temporal/retrieval fields, denied features or training eligibility are introduced.
