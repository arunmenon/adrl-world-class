...
class Probe:
    def __init__(self, fixture: dict[str, Any], output: Path) -> None:
        ...
        self.bound_profile = self.current_profile()
        self.sequence = 0
        self.save("engine-profile.json", self.bound_profile)
    ...
    def checkpoint(self, event: str, value: dict[str, Any]) -> None:
        self.sequence += 1
        self.save(
            f"checkpoint-{self.sequence:04d}.json",
            {"event": event, "at": datetime.now(UTC).isoformat()} | value,
        )

    def current_profile(self) -> dict[str, Any]:
        return profile(
            self.control.engine(),
            self.control._request("GET", "/version"),
            self.control._request("GET", "/info"),
        )
    ...
    def create(self) -> str:
        ...
        assert (
            sum(r["event"] == "create_issued" for r in records)
            < self.fixture["max_containers"]
        )
        assert self.current_profile() == self.bound_profile
        ...
        self.record(
            {
                ...
                "name": name,
                "body": body,
                "engine_profile": self.bound_profile,
            }
        )
        try:
        ...
        self.save(identity + ".created.json", value)
        self.control.verify_created(value, name, body)
        self.owned[identity]["original"] = value
        self.owned[identity]["original_digest"] = digest(value)
        compare(value, value, self.bound_profile, self.current_profile(), digest(value))
        return identity

    def inspect(self, identity: str) -> dict[str, Any]:
        assert identity in self.owned
        value = self.control.inspect(identity)
        self.checkpoint("inspect", {"id": identity, "inspection": value})
        ...
        self.checkpoint("identity_accepted", {"id": identity, "comparison": receipt})
        return value
    ...
    def remove(self, identity: str) -> None:
        try:
            ...
            self.control._request(
                "DELETE", f"/containers/{identity}?force=true&v=false"
            )
        try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            class Handler(BaseHTTPRequestHandler):
                ...
                def do_POST(self) -> None:
                    ...
                    except Exception as exc:  # noqa: BLE001 - retained uncertain request
                        result["upstream_status"] = 500
                        result["upstream_error"] = type(exc).__name__
                    else:
            ...
            try:
                ...
                try:
                    ...
                    assert drained.wait(3), "upstream_request_not_drained"
                finally:
    ...
    def case(self, case: str, identity: str) -> dict[str, Any]:
        started = time.monotonic()
        if case == "identity_start_stop":
            before = self.inspect(identity)
            self.start(identity)
            self.ready(identity)
            active = self.inspect(identity)
            assert active["State"]["Running"]
            assert before["HostConfig"]["OomKillDisable"] is False
            assert active["HostConfig"]["OomKillDisable"] is None
            assert self.kill(identity) == 204
            ended = self.exited(identity)
            outcome = {
                "before_oom": False,
                "active_oom": None,
                "end_oom": ended["HostConfig"]["OomKillDisable"],
                "state": ended["State"],
            }
        elif case == "kill_before_delayed_start":
            ...
            with self.delayed(identity) as (gate, future, transport):
                ...
                future.result(timeout=3)
            assert (
                transport["client_status"] == 404
                and transport["upstream_status"] == 404
            )
...
"""
            ...
            duration = datetime.fromisoformat(
                ...
                value["State"]["StartedAt"].replace("Z", "+00:00")
            )
    ...
    def observe(self, case: str) -> dict[str, Any]:
        ...
        owned_before = set(self.owned)
        self.checkpoint("case_begin", result)
        try:
            identity = self.create()
            result["resource_id"] = identity
            result["completed_assertions"] = self.case(case, identity)
            self.checkpoint("case_assertions_completed", result)
        except BaseException as exc:  # noqa: BLE001 - preserve evidence, then refuse case
            result["primary_error"] = {
                "type": type(exc).__name__,
                "detail": str(exc)[:200],
            }
            self.checkpoint("case_primary_failure", result)
        finally:
            for identity in set(self.owned) - owned_before:
                try:
                    self.remove(identity)
                except BaseException as exc:  # noqa: BLE001 - retain cleanup separately
            ...
            result["status"] = (
                "accepted"
                if result.get("completed_assertions")
                and not result["primary_error"]
                and not result["cleanup_errors"]
                else "failed"
            )
            self.checkpoint("case_final", result)
        if result["status"] != "accepted":
            raise RuntimeError("case_failed_see_preserved_record")
        return result

    def run(self, selection: str = "all") -> None:
        cases = [
            "identity_start_stop",
        ...
        if selection == "identity":
            cases = cases[:1]
        elif selection == "lifecycle":
            cases = cases[1:]
        result = {
            "schema_version": "adrl-launch-recovery-observations-v2",
            "engine": self.engine,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--selection", choices=["identity", "lifecycle"], required=True)
    args = parser.parse_args()
    Probe(json.loads(args.fixture.read_text()), args.output).run(args.selection)
